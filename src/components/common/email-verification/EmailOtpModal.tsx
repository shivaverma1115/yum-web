...
export default function EmailOtpModal({
  ...
  const handleVerify = async () => {
    ...
    if (code.length !== EMAIL_OTP_LENGTH) {
      setError(`Enter the ${EMAIL_OTP_LENGTH}-digit OTP.`);
      return;
    }
    ...
    >
      <div className="w-full max-w-md overflow-hidden rounded-xl bg-white shadow-xl dark:bg-default-50">
        <div className="flex items-center justify-between border-b border-default-200 px-4 py-3">
          <h3 id="email-otp-title" className="text-base font-medium text-default-900">
        ...
        <div className="space-y-4 p-4">
          <p className="text-sm text-default-600">
            ...
            <span className="font-medium text-default-800">{email}</span>.
          </p>

          <div>
            <label htmlFor="email-otp-input" className="mb-2 block text-sm text-default-700">
              OTP
            </label>
            <input
              id="email-otp-input"
